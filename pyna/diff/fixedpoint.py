...
def Newton_discrete(R, Z, Phi, BR, BZ, BPhi, x0_RZPhi, h=1.0, epsilon=1e-5, output_trace=True):
    xRZ_trace = [np.asarray(x0_RZPhi[:-1])]
    while len(xRZ_trace)==1 or np.linalg.norm( xRZ_trace[-2] - xRZ_trace[-1] ) > epsilon:
        RZdiff_sols = RZ_partial_derivative_of_map_4_Flow_Phi_as_t(R, Z, Phi, BR, BZ, BPhi, [x0_RZPhi[-1], x0_RZPhi[-1] + 2*np.pi], xRZ_trace[-1], highest_order=1)
