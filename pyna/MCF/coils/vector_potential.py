...
guaranteed to be divergence-free (∇·B = 0) to machine precision,
...
class VectorPotentialField(VacuumCoilField):
    ...
    (R, Z, φ) grid, then interpolated for evaluation at arbitrary points.
    ...
    Phi : 1D array, shape (nPhi,)
        Toroidal angle grid (rad), must span [0, 2π) uniformly.
    AR, AZ, APhi : 3D array, shape (nR, nZ, nPhi)
        Vector potential components (T·m).
    """
