...
def _natural_field_func(stellarator):
    ...
    def field_func(R, Z, phi):
        ...
        # unit_vec = (dR/ds, dZ/ds, dphi/ds) — not field components directly
        # We reconstruct B from the stellarator internals
...
def island_suppression_current(
    ...
    Solves:
        minimize  |b̃_mn_natural + b̃_mn_coil(I)|²
        subject to |I_k| ≤ I_max  for all k

    Uses a linear response model: b̃_mn_coil(I) = sum_k I_k * b̃_mn^(k),
    where b̃_mn^(k) is the response per unit current of coil k.
    ...
    I_max : float
    n_theta, n_phi : int
        Grid resolution for Fourier integration.
    ...
    # --- natural perturbation amplitude at target surface ---
    ...
    # --- response matrix: amplitude per unit current for each coil ---
    # b̃_total(I) = b_nat + R @ I  →  minimize |b_nat + R @ I|²
    ...
    for k in range(N_coils):
        # Set coil k to 1 A, all others to 0
        for j in range(N_coils):
            control_coils.coils[j] = (control_coils.coils[j][0],
                                       1.0 if j == k else 0.0)
    ...
    # Optimal current: minimize |b_nat + R·I|²
    # For complex linear system: separate real/imag
    # b_nat + R·I = 0  →  least-norm solution
    # [Re(R); Im(R)] I = [-Re(b_nat); -Im(b_nat)]
    A = np.vstack([R_mat.real.reshape(1, -1), R_mat.imag.reshape(1, -1)])
    b = np.array([-b_nat.real, -b_nat.imag])
    ...
    def gradient(I_vec):
        residual = b_nat + R_mat @ I_vec
        # d/dI = 2 * Re(R_k^* * residual)
        return 2.0 * (R_mat.real * residual.real + R_mat.imag * residual.imag)

    bounds = Bounds(lb=-I_max, ub=I_max)
    I0_vec = np.zeros(N_coils)
    result = minimize(objective, I0_vec, jac=gradient, bounds=bounds,
                      method='L-BFGS-B',
    ...
    # --- Monitor side effects on other modes ---
    if monitor_modes:
        for (mm, mn) in monitor_modes:
            ...
            # Response of monitor mode to each coil
...
def phase_control_current(
    ...
    """Find currents to shift the island chain phase by desired_phase_shift (radians).

    The phase of the island O-point is determined by arg(b̃_mn).
    We want: arg(b_nat + R·I) = arg(b_nat) + desired_phase_shift
    ...
    target_m, target_n : int
    desired_phase_shift : float
        Phase rotation in radians (positive = counter-clockwise).
    I_max : float
    n_theta, n_phi : int
    ...
    # Target: b_target = |b_nat| * exp(i * (phase_nat + desired_phase_shift))
    ...
    bounds = Bounds(lb=-I_max, ub=I_max)
    result = minimize(objective, np.zeros(N_coils), jac=gradient, bounds=bounds,
                      method='L-BFGS-B',
                      options={'maxiter': 1000, 'ftol': 1e-15})
...
def multi_mode_control(
    ...
    Minimizes the weighted sum of |b̃_mn|² across all target modes.
    This directly addresses the 按下葫芦起了瓢 problem by jointly
    optimizing for all modes of concern.
    ...
    weights : list of float or None
        Relative weight for each mode (default: all equal).
    I_max : float
    n_theta, n_phi : int
    ...
    # Build response matrix for all modes
    b_nat_all = {}
    R_mat_all = {}
