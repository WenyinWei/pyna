...
class XPointState:
    ...
    DPm: np.ndarray                   # shape (2,2); exp(2ÏA) under axisymmetry
    ...
    Greene_residue: float = 0.0       # (2 â?Tr(DPm)) / 4
...
class OPointState:
    ...
    DPm_eigenvalues: np.ndarray       # shape (2,); |Î»|=1 for elliptic
    iota: float = 0.0                 # rotation transform Î¹ = 1/q
    q: float = float('inf')           # safety factor q = 1/Î¹
...
class TopologyState:
    ...
    phi_ref : float
        Reference toroidal angle for the PoincarÃ© section (default 0).
    """
...
def compute_topology_state(
    ...
    is_axisymmetric : bool
        Use closed-form exp(2ÏA) for DPm.
    phi_ref : float
