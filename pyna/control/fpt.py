...
(no ?-integration needed), enabling sub-millisecond real-time computation.



For 3D configurations (stellarators, RMP tokamaks), the full ?-integration
...
DPm      : Jacobian of full-period Poincar? map

             axisymmetric: DPm = exp(2??A)  [exact, O(1) cost]

             general:      integrate dDPm/dphi = [A, DPm]  [O(N) cost]

?x_cyc   : X/O-cycle shift under perturbation

             axisymmetric: ?x_cyc = -A???? ?(R?B_pol/B_phi)  [exact]

?DPm     : DPm change under perturbation (affects L_c, detachment)

?X^{u/s} : Stable/unstable manifold shift (LCFS shift)

??(?,r)  : Flux surface deformation
...
from pyna.topo._rk4 import rk4_integrate as solve_ivp  # noqa: F401 – kept for API compat
...
def A_matrix(field_func: Callable, R: float, Z: float,
    ...
    A : ndarray, shape (2,2)
        ...
        A[0,0] = ??R?BR/Bphi)/??,  A[0,1] = ??R?BR/Bphi)/??

        A[1,0] = ??R?BZ/Bphi)/??,  A[1,1] = ??R?BZ/Bphi)/??
...
def DPm_axisymmetric(A: np.ndarray, m_turns: float = 1.0) -> np.ndarray:
    ...
    """Compute DPm = exp(2??m?A) for axisymmetric equilibrium.



    Under axisymmetry, A is constant along any X/O orbit, so

    the full-period Poincar? map Jacobian is exactly exp(2??A).
    ...
    DPm : ndarray, shape (2,2)
        ...
        Poincar? map Jacobian.  det(DPm) = 1 (area-preserving).
...
def cycle_shift(A: np.ndarray, delta_g: np.ndarray) -> np.ndarray:
    ...
    """Compute X/O-cycle shift under perturbation ?B (axisymmetric).



    ?x_cyc = -A???? ?(R?B_pol/B_phi)|_{x_cyc}



    Closed-form result from integrating the constant-coefficient

    inhomogeneous ODE for ?X_pol and applying the periodic orbit condition.
    ...
    delta_g : ndarray, shape (2,)
        ...
        Perturbation of g = (R?BR/Bphi, R?BZ/Bphi) at the cycle position.
          ...
          delta_g[0] = R??BR/Bphi - R?BR??Bphi/Bphi?

          delta_g[1] = R??BZ/Bphi - R?BZ??Bphi/Bphi?
...
def delta_g_from_delta_B(
    ...
    """Compute ?(R?B_pol/B_phi) from explicit field perturbation ?B.



    ?(R?BR/Bphi) = R??BR/Bphi - R?BR??Bphi/Bphi?

    ?(R?BZ/Bphi) = R??BZ/Bphi - R?BZ??Bphi/Bphi?
...
def DPm_change(A: np.ndarray, delta_A: np.ndarray,
    ...
    """Compute ?DPm under perturbation (axisymmetric).



    ?DPm = ???? exp(??2??A) ? ?(2??A) ? exp((1-?)?2??A) d?



    Uses Gauss-Legendre quadrature over ? ??[0,1].
    ...
    delta_A : ndarray, shape (2,2)
        ...
        Total variation of A = ?A_direct + ?A_indirect.

          ?A_direct   = ??/?? ? ?B  (local field change at x_cyc)

          ?A_indirect = (?x_cyc ? ????R,Z)) A  (chain rule via cycle shift)
...
def delta_A_total(
    ...
    """Compute total ?A = ?A_direct + ?A_indirect at cycle position.



    ?A_direct   : perturbation to A from ?B at the (unperturbed) cycle position

    ?A_indirect : chain-rule correction due to cycle shift ?x_cyc
    ...
    delta_field_func : callable
        ...
        **Deprecated / ignored when perturbed_field_func is supplied.**
        ...
        direction vectors (not raw ?B components).
...
def manifold_shift(
    ...
    """Compute shift of stable/unstable manifold under perturbation ?B.



    Solves the linear ODE (?-parameterized):

      d/d? (?X^{u/s}) = ?{ ?B_pol + ??_pol/??R,Z) ? ?X^{u/s} }



    with initial condition ?X^{u/s}(?=0) = ?x_cyc.
    ...
    manifold_points : ndarray, shape (N, 2)
        ...
        (R, Z) points along the manifold (may be equally spaced in ? or arc).
    ...
    stable : bool
        ...
        True for stable manifold (sign = ??), False for unstable (sign = +1).
    ...
    for i in range(N - 1):
        ...
        # ?B_pol (approximate: first-order ratio perturbation)
...
def flux_surface_deformation(
    ...
    """Compute flux surface deformation ??(?,r) under perturbation.



    Solves the linearized Poincar? equation:

      ?P^k(?(?,r)) + DP^k(?)???(?,r) = ??(? + k???, r)



    for ?? by representing it in a Fourier basis and solving the
    ...
    poincare_func : callable
        ...
        P^k(x0) ??x_final (k-turn Poincar? map).
    ...
    delta_pk_func : callable
        ...
        ?P^k(x0) ??2-vector, endpoint shift of k-turn orbit.
    ...
    delta_chi : ndarray, shape (N_theta, 2)
        ...
        (?R, ?Z) deformation at each ? grid point.
    ...
    (?? ???x_cyc) as the zeroth-order solution.  A full implementation
    ...
    # Fourier basis: [1, cos ?, sin ?, cos 2?, sin 2?, ...]
