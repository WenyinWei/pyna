...
def magnitude(v: CylindricalVectorField) -> np.ndarray:
    """Pointwise magnitude |V| = sqrt(VR² + VZ² + VPhi²)."""
    return np.sqrt(v.VR**2 + v.VZ**2 + v.VPhi**2)


def cross(v1: CylindricalVectorField, v2: CylindricalVectorField) -> CylindricalVectorField:
...
def divergence(v: CylindricalVectorField) -> CylindricalScalarField:
...
def directional_derivative_of_scalar(
    v: CylindricalVectorField, s: CylindricalScalarField
) -> CylindricalScalarField:
...
def directional_derivative_of_vector(
    v1: CylindricalVectorField, v2: CylindricalVectorField
