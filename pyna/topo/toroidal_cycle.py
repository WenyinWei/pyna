...
This module is explicitly for toroidal field-line dynamics parameterised by
phi. It stores sampled toroidal periodic-orbit traces and provides Newton
search utilities in (R, Z, phi).
"""
...
@dataclass
class ToroidalPeriodicOrbitTrace:
    ...
    def Jac(self) -> np.ndarray:
        ...
        warnings.warn(
            "ToroidalPeriodicOrbitTrace.Jac is deprecated; use ToroidalPeriodicOrbitTrace.DPm instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        return self.DPm
    ...
    def diagnostics(self) -> dict:
        ...
        return {
            'invariant_type': 'ToroidalPeriodicOrbitTrace',
...
def _try_find_cycle_from_seed(
    ...
    for _ in range(max_iter):
        ...
        if np.linalg.norm(G) < tol:
            ...
            return ToroidalPeriodicOrbitTrace(
                rzphi0=rzphi.copy(),
                period_m=n_turns,
                trajectory=traj,
                DPm=DPm,
...
def find_all_cycles_near_resonance(
    ...
    found_orbits: list[ToroidalPeriodicOrbitTrace] = []
