"""Island O/X-point extraction from Poincaré scatter data.

Provides
--------
* :class:`IslandChain` — dataclass holding O/X points and widths.
* :func:`extract_island_width` — infer island geometry from Poincaré data.
"""
...
class IslandChain:
    ...
    half_width_psi : float
        Average half-width in normalised ψ coordinate.
    """
...
def extract_island_width(
    ...
    """Extract O/X points and island half-widths from Poincaré scatter data.

    Algorithm
    ---------
    1. Compute angles from magnetic axis for each Poincaré point.
    ...
    angles = np.arctan2(Z_pts - Z_axis, R_pts - R_axis)  # in (-π, π]
    # Normalise to [0, 2π)
