...
for it in range(6):
    ...
    sol = solve_ivp(aug_rhs, phi_span, y0, method='DOP853', rtol=1e-10, atol=1e-12)
    x_end = sol.y[:2, -1]
    Jac = sol.y[2:, -1].reshape(2, 2)
    res = x_end - x
    det = np.linalg.det(Jac)
    lam = sorted(np.abs(np.linalg.eigvals(Jac)))
    print(f'iter {it}: R={x[0]:.7f} Z={x[1]:.7f}  |res|={np.linalg.norm(res):.2e}  det={det:.8f}  lam_u={lam[1]:.4f}')
