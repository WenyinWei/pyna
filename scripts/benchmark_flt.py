...
# ---------------------------------------------------------------------------
# Solov'ev equilibrium — analytic field function (for CPU tracer)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
print("Running CPU serial (n_workers=1)…")
...
print(f"  Serial:   {t_serial:.3f}s  (speedup 1.00×)")

# ---------------------------------------------------------------------------
# 2. CPU parallel ThreadPool (n_workers=8)
# ---------------------------------------------------------------------------
print("Running CPU parallel ThreadPool (n_workers=8)…")
...
print(f"  Parallel: {t_parallel:.3f}s  (speedup {speedup_parallel:.2f}×)")
...
try:
    ...
    print(f"  CUDA:     {t_cuda:.3f}s  (speedup {speedup_cuda:.2f}×)")
...
for key, v in results.items():
    if v['time'] is None:
        ...
        lines.append(f"  {key:<22}  {v['time']:.4f}s   speedup {v['speedup']:.2f}×")
lines.append("=" * 55)
