...
helper 层在这个层次周围增加一个面向用户的 workflow facade，以及显式扩展点。
...
这个 facade 刻意保持很薄。它不引入新的数学；它让 notebook 代码更可读，同时仍保持
每一次提升都显式可见。
...
adapter 规范化表示；它们不应隐藏数学声明。例如，开放采样轨迹仍是
...
当工作流从多个低层材料装配拓扑对象时，优先使用 builder，因为它们集中处理验证、
metadata 和 back-link。

Bridge
------

``CoreSectionCutBridge`` 是 core 对象默认的连续到离散 bridge：
...
``GeometryFactory`` 通过 builder 层构建拓扑几何。它适合配置驱动示例，以及需要稳定
构造键的下游包。
...
- 把 registry 视为可变状态。在测试和下游包中需要隔离时，使用局部 ``Registry``
  实例。
